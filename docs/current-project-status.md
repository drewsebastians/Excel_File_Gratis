...
Readiness is tracked in `docs/draft-content-readiness.csv`; the repeatable structural audit and open-gate report are generated by `pnpm run audit:drafts` into `docs/qa/draft-readiness-audit.json`. The publication definitions and release-wave proposal are in `docs/draft-publication-governance.md`.
